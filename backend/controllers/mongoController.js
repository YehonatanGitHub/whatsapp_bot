...
const { aggregate } = require('../models/groupModel');

// @desc    statsAllMsgs - get all stats of all mssages in all groups
// @route   GET /api/db/stats-all-msgs
// @access
const statsAllMsgs = asyncHandler(async (req, res) => {
  // const = allfetchMessages()

  // const stats = await MessageInGroup.find({});
  // if (!stats) {
  //   res.status(400);
  //   throw new Error('Stats not found');
  // }
  const newStatsObj = await MessageInGroup.aggregate([
    {
      $group: {
        _id: {
          $dateToString: {
  ...
  // const newStatsObj = await stats.map((message) => ({
  //   group: message.message.from,
  //   date: message.dateCreated,
  // }));

  res.status(200).json(newStatsObj);
});

// @desc    set a group to folow
...
// @desc    all Groups Info (from whatsapp)- get basic info: group name, id, participants
...
const sendMessageToGroup = asyncHandler(async (req, res) => {
  // const sendMessage = async () => {
  const final_number = '120363044957865477@g.us';
